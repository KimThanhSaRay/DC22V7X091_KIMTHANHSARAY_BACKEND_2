...
exports.findAll = async (req, res, next) =>{
	...
	try{
		...
		return next(
			new ApiError(500, "An error orccurred while retrieving contacts")
		);
	}
...
exports.deleteAll = async(req, res, next) => {
	try {
		...
		return next (
				new ApiError(
					500,
					"An error ocuurred while removing all contacts"
					)
		);
	}
};
exports.findAllFavorite = async(req, res, next) => {
	try {
		...
		return next (
				new ApiError(
					500,
					"An error ocuurred while retrieving favorite contacts"
					)
		);
	}
};